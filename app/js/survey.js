...
export const surveyModule = {
    ...
    questions: [
        {
            id: 1,
            text: '수업 전, AI·디지털 도구 사용법을 학생들에게 얼마나 설명하셨나요?',
            options: [
                { text: '수업 시간의 절반 이상을 설명에 썼다', value: 'explain' },
                { text: '10분 정도 간단히 설명했다', value: 'normal' },
                { text: '설명 없이 바로 활동을 시작했다', value: 'none' },
                { text: '사전에 영상으로 대체했다', value: 'video' }
            ]
        },
        {
            id: 2,
            text: '수업 중 기술적 문제(로그인, 오류, 인터넷 등)가 발생했을 때 어땠나요?',
            options: [
                { text: '수업 시간의 절반이 트러블슈팅이었다', value: 'tech' },
                { text: '약간의 문제가 있었지만 해결했다', value: 'some' },
                { text: '기술적 문제는 거의 없었다', value: 'none' },
                { text: '문제 해결을 기다리다 수업이 끝났다', value: 'wait' }
            ]
        },
        {
            id: 3,
        ...
        {
            id: 4,
            text: 'AI 도구가 예상과 다른 결과(환각, 엉뚱한 답변 등)를 냈을 때 어떻게 대응하셨나요?',
            options: [
                { text: '당황해서 즉각 대응하지 못했다', value: 'panic' },
                { text: 'AI도 틀릴 수 있다고 설명하며 넘어갔다', value: 'explain' },
                { text: '그런 상황이 발생하지 않았다', value: 'none' },
                { text: '학생들이 "AI 이상해요!"라며 혼란스러워했다', value: 'chaos' }
            ]
        },
        {
            id: 5,
            text: '수업에서 AI·디지털 도구를 선택한 가장 큰 이유는?',
            options: [
                { text: '학습 목표 달성에 효과적일 것 같아서', value: 'goal' },
                { text: '요즘 트렌드라서 / 신기해서', value: 'trend' },
                { text: '수업 준비가 편할 것 같아서', value: 'easy' },
                { text: '학생들이 좋아할 것 같아서', value: 'student' }
            ]
        },
        {
            id: 6,
            text: '계획한 활동을 시간 내에 완료하셨나요?',
            options: [
                { text: '절반도 못하고 "다음 시간에 이어서"라고 했다', value: 'fail' },
                { text: '급하게 마무리해서 정리 시간이 없었다', value: 'rush' },
                { text: '대체로 계획대로 진행되었다', value: 'ok' },
                { text: '예상보다 빨리 끝나서 시간이 남았다', value: 'early' }
            ]
        },
        {
            id: 7,
            text: '수업 중 나(교사)와 학생들의 에너지 레벨은?',
            options: [
                { text: '나만 신나고 학생들은 멍했다', value: 'alone' },
                { text: '둘 다 신났다', value: 'both' },
                { text: '둘 다 지쳐있었다', value: 'tired' },
                { text: '학생들만 신나고 나는 정신없었다', value: 'student' }
            ]
        },
        {
            id: 8,
            text: '이 수업을 다시 한다면 가장 먼저 바꾸고 싶은 것은?',
            options: [
                { text: '시간 배분 (도구 설명 줄이기)', value: 'time' },
                { text: '도구 선택 (더 간단한 도구로)', value: 'tool' },
                { text: '학습 목표 명확화', value: 'goal' },
                { text: '사전 준비 (학생 기초 역량 확인)', value: 'prep' }
            ]
        }
    ],
    ...
    renderQuestions() {
        ...
        this.questions.forEach(q => {
            ...
            const optionsHtml = q.options.map(opt => 
                `<button class="option-btn" onclick="surveyModule.answer(${q.id}, '${opt.value}')">${opt.text}</button>`
            ).join('');
    ...
    answer(qNum, type) {
        ...
        switch(qNum) {
            case 1:
                if (type === 'explain') this.scores.explain += 3;
                if (type === 'none') this.scores.ai += 1;
                break;
            case 2:
                if (type === 'tech') this.scores.tech += 3;
                if (type === 'wait') { this.scores.loading += 2; this.scores.tech += 1; }
                break;
            case 3:
                if (type === 'tool') this.scores.what += 3;
                if (type === 'confused') this.scores.what += 2;
                if (type === 'excited') this.scores.tool += 1;
                break;
            case 4:
                if (type === 'panic') this.scores.ai += 2;
                if (type === 'chaos') this.scores.ai += 3;
                break;
            case 5:
                if (type === 'trend') this.scores.tool += 3;
                if (type === 'easy') this.scores.tool += 1;
                if (type === 'student') this.scores.alone += 1;
                break;
            case 6:
                if (type === 'fail') this.scores.time += 3;
                if (type === 'rush') this.scores.time += 1;
                break;
            case 7:
                if (type === 'alone') this.scores.alone += 3;
                if (type === 'tired') this.scores.tech += 1;
                break;
            case 8:
                if (type === 'time') this.scores.explain += 1;
                if (type === 'tool') this.scores.tool += 1;
                if (type === 'prep') this.scores.tech += 1;
                break;
        }
    ...
    updateProgress() {
        ...
        const percent = (this.currentQuestion / this.totalQuestions) * 100;
